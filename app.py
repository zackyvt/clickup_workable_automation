...
def candidate_created():
    ...
    task_id = create_candidate_task(data["name"], "" if data["headline"] is None else data["headline"], data["phone"], data["email"], data["address"], data["stage"], "" if data["summary"] is None else data["summary"], data["profile_url"], "" if data["resume_url"] is None else data["resume_url"], data["location"]["location_str"], data["job"]["title"], data["skills"])
